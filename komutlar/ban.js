...
exports.run = (client, message, args) => {
  ...
  if(!message.member.roles.has(`809158355004293120`)) {
  let paradox = new Discord.RichEmbed()
  .setColor('RED')
  .setDescription(`Bu Komutu Kullana Bilmen İçin Ban Hammer Rolü Alman Lazım.
`) 
return message.channel.send(paradox)
  ...
  if (!message.guild.member(user).bannable) return message.reply('HOOOP,YETKİLİLERİ BANLAYAMAM PUŞT.');
